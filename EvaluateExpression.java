...
//由于Java的简单类型不能够精确的对浮点数进行运算，这个类用java.math包中提供的API类BigDecimal提供精确的浮点数运算，包括加减乘除
class Arith {
	...
	public static double add(double v1, double v2) {
		BigDecimal b1 = new BigDecimal(Double.toString(v1));//被加数
		BigDecimal b2 = new BigDecimal(Double.toString(v2));//加数
		return b1.add(b2).doubleValue();
	}
 
	//减法
	public static double sub(double v1, double v2) {
		BigDecimal b1 = new BigDecimal(Double.toString(v1));//被减数
		BigDecimal b2 = new BigDecimal(Double.toString(v2));//减数
		return b1.subtract(b2).doubleValue();
	}
 
	//乘法
	public static double mul(double v1, double v2) {
		BigDecimal b1 = new BigDecimal(Double.toString(v1));//被乘数
		BigDecimal b2 = new BigDecimal(Double.toString(v2));//乘数
		return b1.multiply(b2).doubleValue();
	}
 
	//除法，当发生除不尽的情况时，精确到小数点以后十位，以后的数字四舍五入。
...
class Expression{
	...
	public String EvaluateExpression()
	{
	    ...
	    NumberStack S2 = new NumberStack(); //S1为操作符栈，S2为操作数栈
	    ...
	    while(c != '=')//传进来的String s 以'='结束
	    {
	         ...
	         if(c == 'g')//log(10)
	         {
        		 ...
        		 Numtemp = Math.log10(Numtemp);//以10为底的对数
	        	 S2.Push(Numtemp);
	        	 ptr += Numbuff.length() + 2;
	         ...
	         if(c == 'n')//ln
	         {
        		 ...
        		 Numtemp = Math.log(Numtemp);//以自然常数e为底的对数
	        	 S2.Push(Numtemp);
	        	 ptr += Numbuff.length() + 2;
